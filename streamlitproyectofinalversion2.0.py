...
zone_dict = pd.Series(zone_gdf['zone'].values, index=zone_gdf['location_id']).to_dict()
...
def predict_demand_with_adjacent(location_id, pickup_weekday, pickup_hour):
    ...
    result = {zone_dict.get(zone, "Unknown Zone"): prediction for zone, prediction in zip(all_zones, predictions)}
    return result
